...
if [ "${PYMOR_PYTEST_MARKER}" == "PIP_ONLY" ] ; then
    ...
    popd
    xvfb-run -a py.test -k pickle -r sxX --pyargs pymortests -c .ci/installed_pytest.ini |& grep -v 'pymess/lrnm.py:82: PendingDeprecationWarning'
    pymor-demo -h
    ...
    xvfb-run -a py.test -k pickle -W once::DeprecationWarning -W once::PendingDeprecationWarning -r sxX --junitxml=test_results_${PYMOR_VERSION}.xml
else
    # this runs in pytest in a fake, auto numbered, X Server
    xvfb-run -a py.test -k pickle -r sxX --junitxml=test_results.xml
fi
