...
def test_append(compatible_vector_array_pair):
    ...
    for ind in valid_inds(v2):
        ...
        assert np.all(c1.almost_equal(c2, ind=list(range(len_v1, len(c1))), o_ind=ind))
        if hasattr(v1, 'data'):
        ...
        assert np.all(c1.almost_equal(c1, ind=list(range(len_v1, len_v1 + len_ind)), o_ind=list(range(len_v1 + len_ind, len(c1)))))
...
def test_append_self(vector_array):
    ...
    assert np.all(c.almost_equal(c, ind=list(range(len_v)), o_ind=list(range(len_v, len(c)))))
    if hasattr(v, 'data'):
