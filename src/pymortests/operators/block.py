...
def test_sparse():
    ...
    block_op = BlockOperator(op)
    block_op_half = BlockOperator([[op[0, 0], None], [None, op[1, 1]]])
    ...
    block_op_2 = BlockOperator(op2)
    (0.5 * block_op + 0.5 * block_op_2).assemble(mu)
    block_op_2_dense = BlockOperator(op2)
    ...
    c = sparse_block_op.as_source_array(mu=mu).to_numpy()
    assert np.allclose(a, b, c)
    a = dense_block_op.as_range_array(mu=mu).to_numpy()
    b = block_op.as_range_array(mu=mu).to_numpy()
    c = sparse_block_op.as_range_array(mu=mu).to_numpy()
    assert np.allclose(a, b, c)
