    ...
    class PlotMainWindow(QWidget):
        ...
        def __init__(self, U, plot, length=1, title=None):
            ...
            if length > 1:
                ...
                toolbar = QToolBar()
                self.a_play = QAction(
                    self.style().standardIcon(QStyle.SP_MediaPlay), "Play", self
                )
                ...
                self.a_toend = QAction(
                    self.style().standardIcon(QStyle.SP_MediaSeekForward), "End", self
                )
                ...
                self.a_step_forward = QAction(
                    self.style().standardIcon(QStyle.SP_MediaSkipForward), "Step", self
                )
                self.a_loop = QAction(
                    self.style().standardIcon(QStyle.SP_BrowserReload), "Loop", self
                )
...
def _launch_qt_app(main_window_factory, block):
    ...
    if (
        block and not getattr(sys, "_called_from_test", False)
...
def stop_gui_processes():
    ...
    kill_procs = {
        p for p in multiprocessing.active_children() if p.pid in _launch_qt_processes
    }
    for p in kill_procs:
            ...
            class PlotWidget(QWidget):
                def __init__(self):
                    ...
                    self.colorbarwidgets = [
                        cbar_widget(self, vmin=vmin, vmax=vmax) if cbar_widget else None
                    ...
                    if legend:
                        for i, plot, colorbar, l in zip(
                            range(len(plots)), plots, self.colorbarwidgets, legend
                        ):
                            ...
                            plot_layout.addLayout(
                                subplot_layout, int(i / columns), (i % columns), 1, 1
                            )
                    else:
                        for i, plot, colorbar in zip(
                            range(len(plots)), plots, self.colorbarwidgets
                        ):
                            if not separate_colorbars or backend == "matplotlib":
                                plot_layout.addWidget(
                                    plot, int(i / columns), (i % columns), 1, 1
                                )
                            else:
                                hlayout = QHBoxLayout()
                                hlayout.addWidget(plot)
                                if colorbar:
                                    hlayout.addWidget(colorbar)
                                plot_layout.addLayout(
                                    hlayout, int(i / columns), (i % columns), 1, 1
                                )
                ...
                def set(self, U, ind):
                    ...
                    for u, plot, colorbar, vmin, vmax in zip(
                        U, self.plots, self.colorbarwidgets, self.vmins, self.vmaxs
                    ):
...
def visualize_matplotlib_1d(
    grid, U, codim=1, title=None, legend=None, separate_plots=False, block=False
):
    ...
    class MainWindow(PlotMainWindow):
        def __init__(self, grid, U, codim, title, legend, separate_plots):
            ...
            U = (
                (U.to_numpy(),)
                if isinstance(U, VectorArrayInterface)
                else tuple(u.to_numpy() for u in U)
            )
            if isinstance(legend, str):
                legend = (legend,)
            assert legend is None or isinstance(legend, tuple) and len(legend) == len(U)

            plot_widget = Matplotlib1DWidget(
                None,
                grid,
                count=len(U),
                vmin=[np.min(u) for u in U],
                vmax=[np.max(u) for u in U],
                legend=legend,
                codim=codim,
    ...
    _launch_qt_app(
        lambda: MainWindow(
            grid, U, codim, title=title, legend=legend, separate_plots=separate_plots
        ),
        block,
    )
