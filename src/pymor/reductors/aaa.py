...
class AAAReductor(BasicObject):
    ...
    See :cite:`NST18` for the algorithm
    ...
    def reduce(self, tol=1e-5, itpl_part=None, rom_form='LTI',  max_itpl=None):
        """Reduce using AAA.
        ...
        while len(self.itpl_part) < max_itpl:
            ...
            self.err_history.append(err / max_Hs)
...
def make_bary_func_derivative(itpl_nodes, itpl_vals, coefs, removable_singularity_tol=1e-14):
    r"""Return function for derivative in barycentric form.

    Returns
    -------
    bary_func
        Rational function in barycentric form.
    """
    def bary_func(s):
        d = s - itpl_nodes
        d_zero = d[np.abs(d) < removable_singularity_tol]
        ...
        if len(d_zero) > 0:
            d_min_idx = np.argmin(np.abs(d))
            g = coefs * (itpl_vals[d_min_idx] - itpl_vals)
            g[d_min_idx] = itpl_vals[d_min_idx]
            h = coefs[d_min_idx] * (itpl_nodes[d_min_idx] - itpl_nodes)
            h[d_min_idx] = coefs[d_min_idx]
            return np.atleast_2d(-np.inner(g, 1 / h))
        di = 1 / d
        di2 = di**2
        N = np.inner(di, coefs * itpl_vals)
        D = np.inner(di, coefs) + 1

        dN = -np.inner(di2, coefs * itpl_vals)
        dD = -np.inner(di2, coefs)
        return np.atleast_2d((dN * D - dD * N) / D**2)

    return bary_func

def make_strictly_proper_bary_func(itpl_nodes, itpl_vals, coefs, removable_singularity_tol=1e-14):
    r"""Return function for barycentric form.
