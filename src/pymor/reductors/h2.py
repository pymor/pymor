...
class OneSidedIRKAReductor(BasicInterface):
    ...
    def _projection_matrix(self, r, sigma, b, c, projection):
        ...
        if self.version == "V":
            self.V = gram_schmidt(
                V, atol=0, rtol=0, product=None if projection == "orth" else fom.E
            )
        else:
            self.V = gram_schmidt(
                W, atol=0, rtol=0, product=None if projection == "orth" else fom.E
            )
...
class TSIAReductor(BasicInterface):
    ...
    def _projection_matrices(self, rom, projection):
        fom = self.fom
        self.V, self.W = solve_sylv_schur(
            fom.A, rom.A, E=fom.E, Er=rom.E, B=fom.B, Br=rom.B, C=fom.C, Cr=rom.C
        )
        if projection == "orth":
...
class TFIRKAReductor(BasicInterface):
    ...
    def reduce(
        ...
        assert (
            ...
            and b.shape == (fom.input_dim, r)
        )
        assert (
            c is None
            or isinstance(c, int)
            or isinstance(c, np.ndarray)
            and c.shape == (fom.output_dim, r)
        )
        assert (
            rom0 is None
            or rom0.order == r
            and rom0.input_dim == fom.input_dim
            and rom0.output_dim == fom.output_dim
        )
        assert sigma is None or rom0 is None
        assert b is None or rom0 is None
        assert c is None or rom0 is None
...
def _poles_and_tangential_directions(rom):
    ...
    if isinstance(rom.E, IdentityOperator):
        poles, Y, X = spla.eig(to_matrix(rom.A, format="dense"), left=True, right=True)
    else:
        poles, Y, X = spla.eig(
...
def _convergence_criterion(data, conv_crit):
    ...
    if conv_crit == "sigma":
        sigma = data[0]
        dist_list = [
            spla.norm((sigma_old - sigma) / sigma_old, ord=np.inf)
            for sigma_old in data[1:]
