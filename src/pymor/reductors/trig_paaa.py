...
class TrigPAAAReductor(BasicObject):
    """Reductor implementing the PAAA algorithm with parameters on sphere.
    ...
    def __init__(self, sampling_values, samples_or_fom, conjugate=True, dist_fctns=None):
        ...
        if isinstance(samples_or_fom, TransferFunction) or hasattr(samples_or_fom, 'transfer_function'):
            ...
            self._parameters = {'p': \
                int(np.sum([1 if np.isscalar(s[0]) else s[0].shape[0] for s in sampling_values]) - 1)}
    ...
    def reduce(self, tol=1e-7, itpl_part=None, max_itpl=None):
        ...
        if max_itpl is None:
            ...
            max_itpl = [min(len(s)-1, max_itpl[i]) for i,s in enumerate(svs)]
...
def full_nd_loewner(samples, svs, itpl_part, dist_fctns):
    ...
    """
    L = nd_loewner(samples, svs, itpl_part, dist_fctns)
    ...
    for i_itpl in itertools.product(*([False, True] for _ in range_S)):
        ...
        dist_fctns0 = {}
        for i,k in enumerate(range_S):
            if i_itpl[k]:
                dist_fctns0[len(dist_fctns0)] = dist_fctns[k]
        ...
        for j in itertools.product(*(itpl_part[k] for k in range_S if not i_itpl[k])):
            ...
            LL = nd_loewner(samples0, svs0, itpl_part0, dist_fctns0)
            L = np.vstack([L, LL @ T_mat])
    return L


def make_bary_func(itpl_nodes, itpl_vals, coefs, dist_fctns, removable_singularity_tol=1e-14):
