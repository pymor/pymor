...
class BestApproximationReductor(ProjectionBasedReductor):
    ...
    def __init__(
        ...
        check_tol=1e-3,
        rom_prefix='BestApproximationReduced',
    ):
    ...
    def build_rom(self, projected_operators, error_estimator):
        if len(self.bases) == 1:
            ...
            RB_space = BlockVectorSpace(NumpyVectorSpace(d) for d in dims)
            ...
            def project_onto_basis(blocked_U):
                ...
                return RB_space.from_numpy(projected_U)
        ...
        rom = ComputeModel(
            ...
            name=f'{self.rom_prefix}{self.fom.name}',
        )
        rom.disable_logging()
        if self.rom_prefix:
            rom.enable_caching('memory', f'{self.rom_prefix}_NumpyBlackBoxModel_{dim}')
        return rom
