...
class ScikitLearnComputeReductor(BasicObject):
    def __init__(
        ...
        *,
        ...
        if isinstance(estimators, skbase.BaseEstimator):
            estimators = {
                compute_id: skbase.clone(estimators) for compute_id in compute_shapes
            }
        assert isinstance(estimators, dict)
        assert all(isinstance(k, str) for k in estimators)
        assert all(k in compute_shapes for k in estimators)
    ...
    def reduce(self, data_size: Optional[Number] = None) -> BlackBoxModel:
            ...
            if data_size > 0:
                ...
                for compute_id, estimator in self._fitted_estimators[data_size].items():
                    ...
                    with self.logger.block(
                        ...
                        estimator.fit(X, Y)
            self._roms[data_size] = self._build_rom(self._fitted_estimators[data_size])
