...
class LRADILyapunovSolverLR(LyapunovSolverLR):
    ...
    def projection_shifts_init(self, A, E, B):
        ...
        """
        rng = new_rng(0)
        for i in range(self.projection_shifts_init_maxiter):
    ...
    def wachspress_shifts_init(self, A, E, B):
        ...
        _, Hl, _ = _arnoldi(InverseOperator(E) @ A, self.wachspress_large_ritz_num, b, False)
        _, Hs, _ = _arnoldi(InverseOperator(A) @ E, self.wachspress_small_ritz_num, b, False)
        ...
        J = int(np.ceil(K / (2*v*np.pi) * np.log(4/self.wachspress_tol)))
        p = np.empty(J)
        for i in range(J):
    ...
    def cycle_shifts(self, A, E, V, Z, prev_shifts):
