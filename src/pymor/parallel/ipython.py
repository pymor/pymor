...
class new_ipcluster_pool(BasicInterface):
    ...
    def __enter__(self):
        ...
        if num_engines is None:
            ...
            if wait > 0:
                self.logger.info(
                    f"Waiting {wait} more seconds for engines to start ..."
                )
                time.sleep(wait)
        else:
            running = len(client)
            while running < num_engines and waited < timeout:
                if waited % 10 == 0:
                    self.logger.info(
                        f"Waiting for {num_engines-running} of {num_engines} engines to start ..."
                    )
                time.sleep(1)
                waited += 1
                running = len(client)
            ...
            if running < num_engines:
                raise IOError(
                    f"{num_engines-running} of {num_engines} IPython cluster engines failed to start"
                )
...
class IPythonPool(WorkerPoolBase):
    ...
    def _apply(self, function, *args, **kwargs):
        return self.view.apply_sync(
            _worker_call_function, function, False, args, kwargs
        )
...
def _worker_call_function(function, loop, args, kwargs):
    ...
    kwargs = {
        k: (_remote_objects[v] if isinstance(v, RemoteId) else v)  # NOQA
