...
def visualize_vista_vectorarray(grid, U, bounding_box=([0, 0], [1, 1]), codim=2, title=None, legend=None,
    ...
    if not isinstance(U, tuple):
        filenames = ['huhu']
        write_vtk(grid, U, filenames[0], codim=codim)
    else:
        filenames = [f'huhu-{i}' for i in range(len(U))]
        for fn, u in zip(filenames, U):
            write_vtk(grid, u, fn, codim=codim)
    return visualize_vista(filenames, bounding_box, codim, title, legend, separate_colorbars, rescale_colorbars,
                           columns, color_map)


def visualize_vista(vtk_filelikes, bounding_box=([0, 0], [1, 1]), codim=2, title=None, legend=None,
                    separate_colorbars=False, rescale_colorbars=False, columns=2, color_map=get_cmap('viridis')):
    import pyvista as pv
    mesh = pv.read(vtk_filelikes[0])
    cpos = mesh.plot()
    return cpos
