...
class ColorBarWidget(QOpenGLWidget):
    ...
    def paintGL(self):
        ...
        gl.glEnd()
        p.endNativePainting()
        p.drawText(round((self.width() - self.vmax_width)/2), round(self.text_ascent), self.vmin_str)
        p.drawText(round((self.width() - self.vmin_width)/2),
                   round(self.height() - self.text_height + self.text_ascent), self.vmax_str)
        p.end()
