...
def text_problem(text="pyMOR", font_name=None):
    ...
    font_list = (
        [font_name]
        if font_name
        else ["DejaVuSansMono.ttf", "VeraMono.ttf", "UbuntuMono-R.ttf", "Arial.ttf"]
    )
    font = None
    for filename in font_list:
        try:
            font = ImageFont.truetype(
                filename, 64
            )  # load some font from file of given size
    ...
    def make_bitmap_function(
        ...
        with NamedTemporaryFile(
            ...
            return BitmapFunction(f.name, bounding_box=[(0, 0), size], range=[0.0, 1.0])
    ...
    background = ConstantFunction(1.0, 2) - LincombFunction(dfs, np.ones(len(dfs)))
    ...
    return StationaryProblem(
        ...
        diffusion=diffusion,
        parameter_space=CubicParameterSpace(diffusion.parameter_type, 0.1, 1.0),
    )
