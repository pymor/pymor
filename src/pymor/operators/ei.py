    ...
    def apply(self, U, mu=None):
        ...
        try:
            if self.triangular:
                ...
                interpolation_coefficients = np.linalg.solve(self.interpolation_matrix, AU.data.T).T
        ...
        if len(self.interpolation_dofs) == 0:
            ...
            try:
                if self.triangular:
                    ...
                    interpolation_coefficients = np.linalg.solve(self.interpolation_matrix, JU.data.T).T
            except ValueError:  # this exception occurs when AU contains NaNs ...
            ...
            if isinstance(J, NumpyVectorArray):
                J = NumpyMatrixOperator(J.data.T, kind=self.kind)
            else:
...
class ProjectedEmpiciralInterpolatedOperator(OperatorBase):
    ...
    def __init__(self, restricted_operator, interpolation_matrix, source_basis_dofs,
        ...
        self.kind = restricted_operator.kind
    ...
    def jacobian(self, U, mu=None):
        ...
        if isinstance(M, NumpyVectorArray):
            return NumpyMatrixOperator(M.data.T, kind=self.kind, solver_options=options)
        else:
            assert not options
