...
class OutputOperator(Operator):
    def __init__(self, operator_dict, coefficient_dict=None, non_linear_rules=None, solver_options=None, name=None):
        ...
        assert operator_dict is not None, "Expected dictionary with operators, got empty dictionary!"
        ...
        # we do not want to check source for constant terms as the input is autogenerated to be a vector of ones with appropriate dimension
        if sources is not None:
            assert len(sources.values()) == 1 or all(list(sources.values())[0] == src for src in list(sources.values())[1:]) or 'linear' in self.operators
            self.source = self.operators['linear'].source if 'linear' in self.operators else list(sources.values())[0]
        elif self.constant:
    ...
    def _prepare_coefficient_dicts(self, operator_dict, coefficient_dict, solver_options, name):
        ...
        for key in keys:
            ...
            ret_dict[key] = LincombOperator(operator_dict[key], coefficient_dict[key], solver_options=solver_options, name=self.name + '_' + key)
    ...
    def jacobian(self, U, mu=None):
        jac_ops = {key: self.operators[key].jacobian(U, mu) if key != 'bilinear' else 2 * self.operators[key].jacobian(U, mu) for key in self.operators}
        options = self.solver_options.get('jacobian') if self.solver_options else None
        return OutputOperator(jac_ops, non_linear_rules=self.non_linear_rules, solver_options=options, name=self.name + '_jacobian')

    def jacobian_as_rhs(self, solution):
        return JacobianOutputOperator(self, solution)
...
class JacobianOutputOperator(OutputOperator):
    def __init__(self, output_op, U, name=None):
        assert isinstance(output_op, OutputOperator)
        assert U in output_op.source
        self.__auto_init(locals())
        self.linear = True if 'non-linear' not in output_op.operators else False
    ...
    def _transform_to_jacobian(self, U, output_op, name):
        ...
        self.source = helper_out_op.source
        self.range = helper_out_op.range
        self.name = name + '_jacobian_rhs' if name is not None else output_op.name + '_jacobian_rhs'

    def apply(self, V, mu=None):
        return self.operator.apply2(self.U, V, mu)

    def as_range_array(self, mu=None):
        arrs = [self.operators[key].as_range_array(mu) for key in self.operators]
        R = arrs[0]
        for a in arrs[1:]:
            R.axpy(1, a)
        return R