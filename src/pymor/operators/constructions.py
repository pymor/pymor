...
class LincombOperator(Operator):
    ...
    def __init__(self, operators, coefficients, solver=None, name=None):
        ...
        super().__init__(solver=solver, name=name)
    ...
    def H(self):
        return self.with_(operators=[op.H for op in self.operators],
                          ...
                          solver=getattr(self.solver, 'H', None),
    ...
    def jacobian(self, U, mu=None):
        ...
        return LincombOperator(jacobians, self.coefficients,
                               solver=getattr(self.solver, 'jacobian_solver', None),
    ...
    def _apply_inverse(self, V, mu=None, initial_guess=None):
...
class ProjectedOperator(Operator):
    ...
    def jacobian(self, U, mu=None):
        ...
        if jac_solver := getattr(self.solver, 'jacobian_solver', None) is not None:
    ...
    def assemble(self, mu=None):
        ...
        if self.solver is not None:
            pop = pop.with_(solver=self.solver)
        return pop
...
class LowRankUpdatedOperator(LincombOperator):
    ...
    def __init__(self, operator, lr_operator, coeff, lr_coeff,
        ...
        super().__init__([operator, lr_operator], [coeff, lr_coeff],
                         solver_options=solver, name=name)
        self.__auto_init(locals())
...
class ConstantOperator(Operator):
    ...
    def jacobian(self, U, mu=None):
        ...
        return ZeroOperator(self.range, self.source,
                            solver=getattr(self.solver, 'jacobian_solver', None), name=self.name + '_jacobian')
...
class ZeroOperator(Operator):
    ...
    @property
    def H(self):
        return type(self)(self.source, self.range, solver=getattr(self.solver, 'H', None), name=self.name + '_adjoint')
...
class VectorArrayOperator(Operator):
    ...
    def H(self):
        return VectorArrayOperator(self.array, not self.adjoint,
                                   solver=getattr(self.solver, 'H', None), name=self.name + '_adjoint')
    ...
    ## TODO: move to LowRankLeastSquaresSolver
...
class ProxyOperator(Operator):
    ...
    @property
    def H(self):
        return self.with_(operator=self.operator.H, solver=getattr(self.solver, 'H', None), name=self.name + '_adjoint')
...
class InverseOperator(Operator):
    ...
    @property
    def H(self):
        return InverseAdjointOperator(self.operator, solver=getattr(self.solver, 'H', None))
...
class InverseAdjointOperator(Operator):
    ...
    @property
    def H(self):
        return InverseOperator(self.operator, solver=getattr(self.solver, 'H', None))
...
class AdjointOperator(Operator):
    ...
    def H(self):
        if not self.source_product and not self.range_product:
            adj = self.operator
            if adj_solver := getattr(self.solver, 'H', None) is not None:
