...
from pymor.tools.import_tools import import_class
...
class Operator(ParametricObject):
    ...
    @defaults('solver')
    def __init__(self, solver=None, name=None):  # TODO: document solver
        if isinstance(solver, str):
            solver = import_class(solver)()
    ...
    def apply_inverse(self, V, mu=None, initial_guess=None, solver=None):  ## TODO: document solver
        ...
        if self.linear and self.source.dim == self.range.dim == 0:
            return self.source.zeros(len(V))

        # if we have a solver, use it
        solver = solver or self.solver
        if solver is not None:
            return solver.apply_inverse(self, V, mu=mu, initial_guess=initial_guess)

        # if the operator implement it's own _apply_inverse, use it
        try:
            return self._apply_inverse(V, mu=mu, initial_guess=initial_guess)
        ...
        if (assembled_op is not self
                and (not isinstance(assembled_op, FixedParameterOperator) or assembled_op.operator is not self)):
            if assembled_op.solver:
                return assembled_op.solver.apply_inverse(assembled_op, V, initial_guess=initial_guess)
            try:
                return assembled_op._apply_inverse(V, initial_guess=initial_guess)
            except NotImplementedError:
                pass

        # if the operator is linear, try converting to a matrix as a last resort
        if self.linear:
            mat_op = self.__convert_to_matrix(assembled_op)
            v = mat_op.range.from_numpy(V.to_numpy())
            i = None if initial_guess is None else mat_op.source.from_numpy(initial_guess.to_numpy())
            u = mat_op.apply_inverse(v, initial_guess=i)
    ...
    def _apply_inverse(self, V, mu=None, initial_guess=None):  ## TODO: document
        raise NotImplementedError

    def apply_inverse_adjoint(self, U, mu=None, initial_guess=None, solver=None):  ## TODO: document solver
        ...
        solver = solver or self.solver
        if solver is not None:
            return solver.apply_inverse_adjoint(self, U, mu=mu, initial_guess=initial_guess)

        # if the operator implement it's own _apply_inverse, use it
        try:
            return self._apply_inverse_adjoint(U, mu=mu, initial_guess=initial_guess)
        except NotImplementedError:
            pass
        ...
        if (assembled_op is not self
            ...
            if assembled_op.solver is not None:
                return assembled_op.solver.apply_inverse_adjoint(assembled_op, U, initial_guess=initial_guess)
            try:
                return assembled_op._apply_inverse_adjoint(U, initial_guess=initial_guess)
            except NotImplementedError:
                pass
        ...
        v = mat_op.apply_inverse_adjoint(u, initial_guess=i)
        return self.range.from_numpy(v.to_numpy())

    def _apply_inverse_adjoint(self, U, mu=None, initial_guess=None):  # TODO: document
        raise NotImplementedError
    ...
    def _assemble_lincomb(self, operators, coefficients, identity_shift=0., solver=None, name=None):
    ...
    def __convert_to_matrix(self, assembled_op):
...
class LinearSolver(Solver):
    ...
    @property
    def H(self):
        return self if self.self_adjoint else AdjointLinearSolver(self)

    @property
    def jacobian_solver(self):
        return self

    def apply_inverse_adjoint(self, operator, U, mu=None, initial_guess=None):
        self.apply_inverse(operator.H, U, mu=mu, initial_guess=initial_guess)


class AdjointLinearSolver(LinearSolver):

    def __init__(self, solver):
        self.solver = solver

    def apply_inverse(self, operator, V, mu=None, initial_guess=None):
        return self.solver.apply_inverse_adjoint(operator.H, V, mu=mu, initial_guess=initial_guess)

    def apply_inverse_adjoint(self, operator, U, mu=None, initial_guess=None):
        return self.solver.apply_inverse(operator.H, mu=mu, initial_guess=initial_guess)
