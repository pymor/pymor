...
def load_gmsh(gmsh_file):
    ...
    faces = np.array(
        [
            [node_ids[nodes[0]], node_ids[nodes[1]], node_ids[nodes[2]]]
    ...
    name_ids = dict(
        zip(
            [pn[0] for pn in sections["PhysicalNames"]],
    ...
    for bt in boundary_types:
        ...
        masks[bt][0][[line_ids[l[0]] for l in sections["Elements"]["line"]]] = [
            (bt == sections["PhysicalNames"][name_ids[l[1][0]]][2])
            for l in sections["Elements"]["line"]
        ]
        ind = np.array(
        ...
        val = masks[bt][0][
            [line_ids[l[0]] for l in sections["Elements"]["line"] for n in l[2]]
        ]
    ...
    logger.info(
        f"Parsing took {t_parse} s; Grid creation took {t_grid} s; BoundaryInfo creation took {t_bi} s"
    )
...
def _parse_gmsh_file(f):
    ...
    def parse_elements(elements):
        try:
            ...
            raise GmshError(
                f"first line of elements sections is not a number: {elements[0]}"
            )
        ...
        def parse_line(fields):
            ...
            return (
                element_type,
                (fields[0], tuple(fields[3 : 3 + num_tags]), fields[3 + num_tags :]),
            )
    ...
    def parse_names(physical_names):
        ...
        try:
            physical_names = [
                (int(b), int(a), str(c).replace('"', "")) for a, b, c in physical_names
            ]
        except ValueError:
            raise GmshError("malformed physical names section")
