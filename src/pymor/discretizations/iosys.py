...
class LTISystem(DiscretizationInterface):
    ...
    p
        Number of outputs.
    operators
        Dictonary of operators A, B, C, D, and E
    """
    linear = True
    ...
    def __init__(self, A, B, C, D=None, E=None, cont_time=True):
        ...
        self.m = B.source.dim
        self.p = C.range.dim
        self.operators = FrozenDict({'A': A,
                                     'B': B,
                                     'C': C,
                                     'D': D if D is not None else ZeroOperator(B.source, C.range),
                                     'E': E if E is not None else IdentityOperator(A.source)})
    ...
    def __add__(self, other):
        ...
        B = BlockOperator.vstack((self.operators['B'], other.operators['B']))
        C = BlockOperator.hstack((self.operators['C'], other.operators['C']))
    ...
    def __mul__(self, other):
        ...
        assert self.operators['B'].source == other.operators['C'].range

        A = BlockOperator([[self.operators['A'], Concatenation(self.operators['B'], other.operators['C'])],
                           [None, other.operators['A']]])
        B = BlockOperator.vstack((Concatenation(self.operators['B'], other.operators['D']),
                                  other.operators['B']))
        C = BlockOperator.hstack((self.operators['C'], Concatenation(self.operators['D'],
                                  other.operators['C'])))
    ...
    def compute_poles(self):
        ...
        if self._poles is None:
            A = to_matrix(self.operators['A'])
            E = None if isinstance(self.operators['E'], IdentityOperator) else to_matrix(self.operators['E'])
    ...
    def compute_gramian(self, typ, subtyp, param=None, me_solver=None, tol=None):
        """Compute a Gramian.
        ...
        param
            Parameter of the Gramian:

            - used for the Bound Real Gramian (then it must be a positive float).
        me_solver
        ...
        if (typ not in self._gramian or subtyp not in self._gramian[typ] or param not in self._gramian[typ][subtyp] or
                tol is not None):
            A = self.operators['A']
            B = self.operators['B']
            C = self.operators['C']
            E = self.operators['E'] if not isinstance(self.operators['E'], IdentityOperator) else None
            if typ == 'lyap':
                assert param is None
                if subtyp == 'cf':
                    self._gramian[typ][subtyp][None] = solve_lyap(A, E, B, trans=False, me_solver=me_solver, tol=tol)
                elif subtyp == 'of':
                    self._gramian[typ][subtyp][None] = solve_lyap(A, E, C, trans=True, me_solver=me_solver, tol=tol)
                else:
                    raise NotImplementedError("Only 'cf' and 'of' subtypes are possible for 'lyap' type.")
            elif typ == 'lqg':
                assert param is None
                if subtyp == 'cf':
                    self._gramian[typ][subtyp][None] = solve_ricc(A, E=E, B=B, C=C, trans=True, me_solver=me_solver,
                                                                  tol=tol)
                elif subtyp == 'of':
                    self._gramian[typ][subtyp][None] = solve_ricc(A, E=E, B=B, C=C, trans=False, me_solver=me_solver,
                                                                  tol=tol)
                else:
                ...
                if subtyp == 'cf':
                    self._gramian[typ][subtyp][param] = solve_ricc(A, E=E, B=B / np.sqrt(typ[2]), C=C / np.sqrt(typ[2]),
                    ...
                    raise NotImplementedError("Only 'cf' and 'of' subtypes are possible for 'br' type.")
            else:
                raise NotImplementedError("Only 'lyap', 'lqg', and 'br' types are available.")

    def compute_sv_U_V(self, typ, param=None, me_solver=None):
        ...
        param
            Parameter of the Gramian (see :func:`~pymor.discretizations.iosys.LTISystem.compute_gramian`).
        me_solver
            Matrix equation solver to use (see
            :func:`pymor.algorithms.lyapunov.solve_lyap` or
            :func:`pymor.algorithms.riccati.solve_ricc`).
        ...
        if (typ not in self._sv or param not in self._sv[typ] or
                typ not in self._U or param not in self._U[typ] or
                typ not in self._V or param not in self._V[typ]):
            self.compute_gramian(typ, 'cf', param=param, me_solver=me_solver)
            self.compute_gramian(typ, 'of', param=param, me_solver=me_solver)

            U, self._sv[typ][param], Vh = spla.svd(self.operators['E'].apply2(self._gramian[typ]['of'][param],
                                                                              self._gramian[typ]['cf'][param]))

            self._U[typ][param] = NumpyVectorArray(U.T)
            self._V[typ][param] = NumpyVectorArray(Vh)
    ...
    def norm(self, name='H2'):
        ...
        if name == 'H2':
            ...
            B, C = self.operators['B'], self.operators['C']

            if 'lyap' in self._gramian and 'cf' in self._gramian['lyap']:
                self._H2_norm = np.sqrt(C.apply(self._gramian['lyap']['cf'][None]).l2_norm2().sum())
            elif 'lyap' in self._gramian and 'of' in self._gramian['lyap']:
                self._H2_norm = np.sqrt(B.apply_adjoint(self._gramian['lyap']['of'][None]).l2_norm2().sum())
            elif self.m <= self.p:
            ...
            jobe = 'I' if isinstance(self.operators['E'], IdentityOperator) else 'G'
            equil = 'S'
            jobd = 'Z' if isinstance(self.operators['D'], ZeroOperator) else 'D'
            A, B, C, D, E = map(to_matrix, (self.operators['A'], self.operators['B'], self.operators['C'],
                                            self.operators['D'], self.operators['E']))
            self._Hinf_norm, self._fpeak = ab13dd(dico, jobe, equil, jobd, self.n, self.m, self.p, A, E, B, C, D)

            return self._Hinf_norm
        elif name == 'Hankel':
            self.compute_sv_U_V('lyap')
            return self._sv['lyap'][None][0]
        else:
    ...
    def project(self, Vr, Wr, Er_is_identity=False):
        ...
        ----------
        Vr
            Right projection matrix, |VectorArray| from `self.operators['A'].source`.
        Wr
            Left projection matrix, |VectorArray| from `self.operators['A'].source`.
        Er_is_identity
        ...
        Cr = VectorArrayOperator(self.operators['C'].apply(Vr))
        Dr = self.operators['D']
    ...
    def bt(self, r=None, tol=None, typ='lyap', param=None, me_solver=None, method='bfsr'):
        ...
        if r is not None and r > min([len(self._gramian[typ]['cf'][param]), len(self._gramian[typ]['of'][param])]):
            raise ValueError('r needs to be smaller than the sizes of Gramian factors.' +
                             ' Try reducing the tolerance in the low-rank Lyapunov equation solver.')

        self.compute_sv_U_V(typ, param=param, me_solver=me_solver)

        if r is None:
            bounds = np.zeros(self._sv[typ][param].shape)
            if typ[0] == 'lyap':
                bounds[:-1] = 2 * self._sv[typ][param][-1:0:-1].cumsum()[::-1]
            elif typ[0] == 'lqg':
                tmp = self._sv[typ][param][-1:0:-1]
                bounds[:-1] = 2 * (tmp / np.sqrt(1 + tmp ** 2)).cumsum()[::-1]
            elif typ[0] == 'br':
                bounds[:-1] = 2 * param * self._sv[typ][param][-1:0:-1].cumsum()[::-1]
            r = np.argmax(bounds <= tol) + 1

        Vr = VectorArrayOperator(self._gramian[typ]['cf'][param]).apply(self._V[typ][param], ind=list(range(r)))
        Wr = VectorArrayOperator(self._gramian[typ]['of'][param]).apply(self._U[typ][param], ind=list(range(r)))

        if method == 'sr':
            alpha = 1 / np.sqrt(self._sv[typ][param][:r])
            Vr.scal(alpha)
            Wr.scal(alpha)
    ...
    def arnoldi(self, sigma, b_or_c):
        ...
        r = len(sigma)
        V = self.operators['A'].source.type.make_array(self.operators['A'].source.subtype, reserve=r)

        v = np.array([1.])
        if b_or_c == 'b':
            v = self.operators['B'].source.from_data(v)
            v = self.operators['B'].apply(v)
        else:
            v = self.operators['C'].range.from_data(v)
            v = self.operators['C'].apply_adjoint(v)
        v.scal(1 / v.l2_norm()[0])

        for i in range(r):
            if sigma[i].imag == 0:
                sEmA = LincombOperator((self.operators['E'], self.operators['A']), (sigma[i].real, -1))
                ...
                sEmA = LincombOperator((self.operators['E'], self.operators['A']), (sigma[i], -1))
    ...
    def arnoldi_tangential(self, sigma, b_or_c, directions):
        ...
        directions
            ...
            `self.operators['B'].source` or `self.operators['C'].range`
            (depending on `b_or_c`).
        ...
        assert (b_or_c == 'b' and self.m > 1 and directions in self.operators['B'].source or
                b_or_c == 'c' and self.p > 1 and directions in self.operators['C'].range)
        ...
        for i in range(r):
            if sigma[i].imag == 0:
                ...
                if b_or_c == 'b':
                    if i == 0:
                        v = sEmA.apply_inverse(self.operators['B'].apply(directions.real, ind=0))
                    else:
                        Bd = self.operators['B'].apply(directions.real, ind=i)
                    ...
                    if i == 0:
                        v = sEmA.apply_inverse_adjoint(self.operators['C'].apply_adjoint(directions.real, ind=0))
                    else:
                        CTd = self.operators['C'].apply_adjoint(directions.real, ind=i)
                ...
                if b_or_c == 'b':
                    if i == 0:
                        v = sEmA.apply_inverse(self.operators['B'].apply(directions, ind=0))
                    else:
                        Bd = self.operators['B'].apply(directions, ind=i)
                        VTBd = VectorArrayOperator(V, transposed=True).apply(Bd)
                        sEmA_proj_inv_VTBd = NumpyMatrixOperator(sEmA.apply2(V, V)).apply_inverse(VTBd)
                        V_sEmA_proj_inv_VTBd = VectorArrayOperator(V).apply(sEmA_proj_inv_VTBd)
                        rd = sEmA.apply(V_sEmA_proj_inv_VTBd) - Bd
                    ...
                    if i == 0:
                        v = sEmA.apply_inverse_adjoint(self.operators['C'].apply_adjoint(directions, ind=0))
                    else:
                        CTd = self.operators['C'].apply_adjoint(directions, ind=i)
                        VTCTd = VectorArrayOperator(V, transposed=True).apply(CTd)
                        sEmA_proj_inv_VTCTd = NumpyMatrixOperator(sEmA.apply2(V, V)).apply_inverse_adjoint(VTCTd)
                        V_sEmA_proj_inv_VTCTd = VectorArrayOperator(V).apply(sEmA_proj_inv_VTCTd)
                        rd = sEmA.apply_adjoint(V_sEmA_proj_inv_VTCTd) - CTd
    ...
    def interpolation(self, sigma, b, c):
        ...
        b
            Right tangential directions, |VectorArray| of length `r` from `self.operators['B'].source`.
        c
            Left tangential directions, |VectorArray| of length `r` from `self.operators['C'].range`.
        ...
        """
        r = len(sigma)
        assert b in self.operators['B'].source and len(b) == r
        assert c in self.operators['C'].range and len(c) == r

        b.scal(1 / b.l2_norm())
        c.scal(1 / c.l2_norm())

        Vr = self.operators['A'].source.type.make_array(self.operators['A'].source.subtype, reserve=r)
        Wr = self.operators['A'].source.type.make_array(self.operators['A'].source.subtype, reserve=r)

        for i in range(r):
            if sigma[i].imag == 0:
                sEmA = LincombOperator((self.operators['E'], self.operators['A']), (sigma[i].real, -1))

                Bb = self.operators['B'].apply(b.real, ind=i)
                Vr.append(sEmA.apply_inverse(Bb))

                CTc = self.operators['C'].apply_adjoint(c.real, ind=i)
                Wr.append(sEmA.apply_inverse_adjoint(CTc))
            elif sigma[i].imag > 0:
                sEmA = LincombOperator((self.operators['E'], self.operators['A']), (sigma[i], -1))

                Bb = self.operators['B'].apply(b, ind=i)
                v = sEmA.apply_inverse(Bb)
                Vr.append(v.real)
                Vr.append(v.imag)

                CTc = self.operators['C'].apply_adjoint(c, ind=i)
    ...
    def irka(self, r, sigma=None, b=None, c=None, tol=1e-4, maxit=100, verbose=False, force_sigma_in_rhp=True,
        ...
        assert b is None or b in self.operators['B'].source and len(b) == r
        assert c is None or c in self.operators['C'].range and len(c) == r

        if sigma is None:
            sigma = np.logspace(-1, 1, r)
        if b is None:
            b = self.operators['B'].source.from_data(np.ones((r, self.m)))
        if c is None:
            c = self.operators['C'].range.from_data(np.ones((r, self.p)))
