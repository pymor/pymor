...
def discretize_gmsh(
    ...
    try:
        version = subprocess.check_output(
            ["gmsh", "--version"], stderr=subprocess.STDOUT, env=env
        ).decode()
    ...
    try:
        ...
        if domain_description is not None:
            ...
            if geo_file_path is None:
                geo_file = tempfile.NamedTemporaryFile(
                    mode="wt", delete=False, suffix=".geo"
                )
            ...
            point_ids = dict(
                zip(
                    [str(p) for ps in points for p in ps],
            ...
            lines = [
                [point_ids[str(p0)], point_ids[str(p1)]]
            ...
            geo_file.write(
                'Physical Surface("boundary") = {' + str(line_loop_ids[0] + 1) + "};\n"
            )
        ...
        if msh_file_path is None:
            msh_file = tempfile.NamedTemporaryFile(
                mode="wt", delete=False, suffix=".msh"
            )
