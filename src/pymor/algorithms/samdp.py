...
def samdp(A, E, B, C, nwanted, init_shifts=None, which='NR', tol=1e-10, imagtol=1e-6, conjtol=1e-8,
          dorqitol=1e-4, rqitol=1e-10, maxrestart=100, krestart=20, rqi_maxiter=10, solver=None):
    ...
    while nrestart < maxrestart:
        ...
        sEmA = st * E - A
        sEmAB = sEmA.apply_inverse(B_defl, solver=solver)
        ...
        x = sEmAB.lincomb(u)
        v = sEmA.apply_inverse_adjoint(C_defl.lincomb(y), solver=solver)
...
def _twosided_rqi(A, E, x, y, theta, init_res, imagtol, rqitol, maxiter, solver):
    ...
    while i < maxiter:
        ...
        tEmA = theta * E - A
        x_rqi = tEmA.apply_inverse(Ex, solver=solver)
        v_rqi = tEmA.apply_inverse_adjoint(Ey, solver=solver)
