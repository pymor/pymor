...
def recalculated_shifted_chol_qr(A, product=None, maxiter=3, offset=0, orth_tol=None, check_finite=True, copy=True):
    r"""Orthonormalize a |VectorArray| using the shifted CholeskyQR algorithm.
    ...
    logger = getLogger('pymor.algorithms.chol_qr.recalculated_shifted_chol_qr')
            ...
            while True:
                ...
                if shift is None:
                    ...
                    if product is None:
                        ...
                        if not eig:
                            from pymor.algorithms.eigs import eigs
                            eig = eigs(product, k=1, tol=1e-2)[0][0]

                        shift = 2*m*np.sqrt(m*n)+n*(n+1)*np.sqrt(np.abs(eig))
                        XX = A[offset:].gramian(product=product)

                    norm = spsla.eigsh(XX, k=1, tol=1e-4, return_eigenvectors=False)[0]
