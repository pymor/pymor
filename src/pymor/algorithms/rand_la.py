...
class RandomizedRangeFinder(BasicObject):
    ...
    failure_tolerance
        Maximum failure probability. Only needed for error_estimator='halko'.
    num_testvecs
        Number of test vectors. Only needed for error_estimator='halko'.
    lambda_min
        The smallest eigenvalue of source_product. Only needed for error_estimator='halko'.
    ...
    error_estimator
        The error estimator used. Either 'halko' (default) or 'loo'.
    """
    ...
    def __init__(self, A, range_product=None, source_product=None, A_adj=None, power_iterations=0, block_size=None,
        ...
        self.estimate_error = self._buhr_estimator if error_estimator == 'halko' else self._loo_estimator
        self.Omega = A.range.empty()  # the test vectors for 'halko' or the drawn samples for 'loo'.
