...
def solve_ricc_lrcf(A, E, B, C, R=None, S=None, trans=False, options=None):
    ...
    while res > Ctol and j < options['maxiter']:
        ...
        if j == 0:
            ...
            V = (L + N.lincomb_TP(ImBNKL)) * np.sqrt(-2 * shifts[j_shift].real)
        ...
        if np.imag(shifts[j_shift]) == 0:
            ...
            if not trans:
                EVYt = E.apply(V).lincomb_TP(spla.inv(Yt).T)  # TODO: simplify?
            else:
                EVYt = E.apply_adjoint(V).lincomb_TP(spla.inv(Yt).T)  # TODO: simplify?
            RF.axpy(np.sqrt(-2*shifts[j_shift].real), EVYt)
            K += EVYt.lincomb_TP(VB)
            j += 1
        else:
            Z.append(V.real)
            Z.append(V.imag)
            ...
            Y = spla.block_diag(Y, Yt)
            if not trans:
                EVYt = E.apply(cat_arrays([V.real, V.imag])).lincomb_TP(spla.inv(Yt).T)  # TODO: simplify?
            else:
                EVYt = E.apply_adjoint(cat_arrays([V.real, V.imag])).lincomb_TP(spla.inv(Yt).T)  # TODO: simplify?
            RF.axpy(np.sqrt(-2 * shifts[j_shift].real), EVYt[:len(C)])
            K += EVYt.lincomb_TP(F2)
            j += 2
        j_shift += 1
        res = np.linalg.norm(RF.gramian(), ord=2)
    ...
    cf = spla.cholesky(Y)
    Z_cf = Z.lincomb_TP(spla.solve_triangular(cf, np.eye(len(Z))))
    return Z_cf
