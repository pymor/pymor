...
def rational_arnoldi(A, E, b, sigma, trans=False, solver=None):
    ...
    for i in range(r):
        ...
        if sigma[i].imag == 0:
            ...
            sEmA = sigma[i] * E - A
        if not trans:
            v = sEmA.apply_inverse(v if len(V) == 0 else E.apply(v), solver=solver)
        else:
            v = sEmA.apply_inverse_adjoint(v if len(V) == 0 else E.apply_adjoint(v), solver=solver)
        if sigma[i].imag == 0:
            V.append(v)
...
def tangential_rational_krylov(A, E, B, b, sigma, trans=False, orth=True, solver=None):
    ...
    for i in range(r):
        if sigma[i].imag == 0:
            ...
            if not trans:
                Bb = B.apply(b.real[i])
                V.append(sEmA.apply_inverse(Bb, solver=solver))
            else:
                BTb = B.apply_adjoint(b.real[i])
                V.append(sEmA.apply_inverse_adjoint(BTb, solver=solver))
        elif sigma[i].imag > 0:
            sEmA = sigma[i] * E - A
            if not trans:
                Bb = B.apply(b[i])
                v = sEmA.apply_inverse(Bb, solver=solver)
            else:
                BTb = B.apply_adjoint(b[i].conj())
                v = sEmA.apply_inverse_adjoint(BTb, solver=solver)
            V.append(v.real)
            V.append(v.imag)
    if orth:
        gram_schmidt(V, atol=0, rtol=0, copy=False)
