...
def assemble_lincomb(operators, coefficients, solver=None, name=None):
    ...
    """
    return AssembleLincombRules(tuple(coefficients), solver, name).apply(tuple(operators))
...
class AssembleLincombRules(RuleTable):
    ...
    def __init__(self, coefficients, solver, name):
    ...
    def action_zero_coeff(self, ops):
        ...
        if len(without_zero) == 0:
            return ZeroOperator(ops[0].range, ops[0].source, solver=self.solver, name=self.name)
        else:
            new_ops, new_coeffs = zip(*without_zero)
            return assemble_lincomb(new_ops, new_coeffs, solver=self.solver, name=self.name)
    ...
    def action_IdentityOperator(self, ops):
        ...
        if coeff == 0:
            return ZeroOperator(ops[0].source, ops[0].source, solver=self.solver, name=self.name)
        else:
    ...
    def action_VectorArrayOperator(self, ops):
        ...
        return VectorArrayOperator(array, adjoint=adjoint, solver=self.solver, name=self.name)
    ...
    def action_SecondOrderModelOperator(self, ops):
        ...
        so_op = SecondOrderModelOperator(alpha, beta, A, B, solver=self.solver)
        return so_op
    ...
    def action_BlockDiagonalOperator(self, ops):
        ...
        if len(ops) > 1:
            for i in range(num_source_blocks):
                ...
                if blocks[i] is None:
                    return None
            return BlockDiagonalOperator(blocks, solver=self.solver)
        else:
            c = coefficients[0]
            if c == 1:
                return ops[0]
        ...
        if len(ops) > 1:
            for (i, j) in np.ndindex(shape):
                ...
                if blocks[i, j] is None:
                    return None
            return operator_type(blocks, solver=self.solver)
        else:
            c = coefficients[0]
            if c == 1:
                return ops[0]
    ...
    def action_merge_low_rank_operators(self, ops):
        ...
        new_low_rank_op = LowRankOperator(left, core, right, inverted=inverted, solver=self.solver)
    ...
    def action_return_lincomb(self, ops):
        return LincombOperator(ops, self.coefficients, solver=self.solver, name=self.name)
