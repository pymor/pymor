...
'''This module contains algorithms for emprirical interpolation of operators.
...
to instantiate an :class:`pymor.operators.ei.EmpiricalInterpolatedOperator`.
...
def ei_greedy(evaluations, error_norm=None, target_error=None, max_interpolation_dofs=None,
    ...
    can be used to instantiate an :class:`pymor.operators.ei.EmpiricalInterpolatedOperator`.
...
class EvaluationProvider(CacheableInterface):
    ...
    caching
        Name of the cache region to use.
    '''
...
def interpolate_operators(discretization, operator_names, parameter_sample, error_norm=None,
    ...
    product
        See :func:`ei_greedy`.
    caching
        Name of the cache region in which the operator evaluations will be stored.

    Returns
    -------
    ei_discretization
        Discretization with opterators given by `operator_names` replaced by
