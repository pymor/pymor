function [X, info] = ml_lyap_sgn(A, G, E, opts)
%ML_LYAP_SGN Continuous-time Lyapunov equation solver.
%
% SYNTAX:
%   [X, info] = ML_LYAP_SGN(A, G)
%   [X, info] = ML_LYAP_SGN(A, G, [])
%   [X, info] = ML_LYAP_SGN(A, G, [], opts)
%
%   [X, info] = ML_LYAP_SGN(A, G, E)
%   [X, info] = ML_LYAP_SGN(A, G, E, opts)
%
% DESCRIPTION:
...
%   1998, pp. 205--210.
%
% See also ml_lyap_sgn_fac, ml_lyap_sgn_ldl.
...
while (niter <= opts.MaxIter) && not(converged)
    ...
    if opts.Info
        fprintf(1, ['LYAP_SGN step: %4d absolute error: %e' ...
            ' relative error: %e \n'], ...
