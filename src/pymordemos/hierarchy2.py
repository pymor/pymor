...
from pymor.models.hierarchy import AdaptiveDDReductor, AdaptiveRBReductor, ModelHierarchy
...
def main(
    ...
    adaptive_rb_reductor = AdaptiveRBReductor(fom, rb_reductor, compression=compression)
    adaptive_dd_reductor = AdaptiveDDReductor(dd_reductor_parameters, retrain_interval=1)

    tol = 5e-3
    hierarchy = ModelHierarchy([adaptive_dd_reductor, adaptive_rb_reductor], tol, fom=fom)
    ...
    # the hierarchy reports the index of the used model into its `models` list, which
    # for this demo is [data-driven, reduced basis, full-order]
    model_labels = ['DD', 'RB', 'FOM']
