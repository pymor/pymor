...
def main(
    ...
    initial_guess = np.array([0.25, 0.5, 0.25, 0.5, 0.25, 0.5])
    ...
    opt_fom_result = minimize(partial(record_results, fom_objective_functional,
                              ...
                              bounds=(range_1, range_2, range_2, range_2, range_2, range_2),
                              options={'ftol': 1e-15})
...
def create_fom():
    rng = new_rng(222)
    functions = [
        ExpressionFunction('(2.5 <= x[0]) * (x[0] <= 3.5) * (0 <= x[1]) * (x[1] <= 4) * 1.', dim_domain=2),
    ...
    coefficients = [
        ProjectionParameterFunctional('k0', 1),
        ProjectionParameterFunctional('k1', 1),
        ProjectionParameterFunctional('k2', 1),
        ProjectionParameterFunctional('k3', 1),
        ProjectionParameterFunctional('k4', 1)
    ]
    ...
    grid, boundary_info = load_gmsh('data/fin_mesh.msh')
    ...
    output_functional = OutputFunctional(
        {'constant': [const_op], 'linear': [boundary_functional.H], 'bilinear': [bilin_op]},
        {'constant': [const_coeff], 'linear': [lin_coeff], 'bilinear': [.5]}
    )
    ...
    range_2 = helper_problem.parameter_space.ranges['k0']
