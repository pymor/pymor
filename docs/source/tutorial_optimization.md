...
compare all methods that we have so far discussed in this notebook.
...
const_op = ConstantOperator(constant_vec, fom.solution_space)

ones = fom.solution_space.ones()
...
def compute_gradient_with_opt_rom(opt_dict, mu):
    opt_rom = opt_dict['opt_rom']
    return opt_rom.output_d_mu(opt_rom.parameters.parse(mu), return_array=True, use_adjoint=adjoint)
...
Now we repeat the previous optimization steps and compare the results. Note that the results obviously differ because our objective has changed!
...
assert fom_result.nit == 3
assert opt_along_path_result.nit == 3
assert opt_along_path_minimization_data['num_evals'] == 8
assert opt_along_path_minimization_data['enrichments'] == 7
assert opt_along_path_adaptively_minimization_data['enrichments'] == 3
```
