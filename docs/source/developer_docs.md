...
it is not required. Once you have `pre-commit` installed in your environment, run
...
files from these specifications, which contain pinned versions of all packages installed into the
respective GitLab CI images.
The extras included in the images are specified in `Makefile`.
...
pyMOR uses [hypothesis](https://hypothesis.works/) for property-based testing.
...
The main responsibility of the `preflight` job is to compute the `sha256sum` of all
...
When needed, updated CI images are built using a podman-in-docker setup.
...
: Commits documentation built in {ref}`ref_gitlab_ci_stage_testbuild`to the
  ...
  A binder setup for the generated tutorial notebooks is added on a branch with a name
  matching the currently checked out git branch of pyMOR.

submit coverage
: This job accumulates all the coverage databases generated by previous stages and submits them
...
This enables GitLab CI runs for those PRs.
...
pyMOR's API changes in a way that is not backward compatible. To help select an appropriate code path,
