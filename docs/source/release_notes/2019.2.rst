...
:class:`~pymor.models.interface.Model` and all deriving classes accordingly
...
and the notion of `special_operators` in :class:`~pymor.models.interface.Model`,
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The :class:`~pymor.vectorarrays.interface.VectorArray` has been extended to
allow the creation of non-zero vectors using the
:meth:`~pymor.vectorarrays.interface.VectorArray.ones` and
:meth:`~pymor.vectorarrays.interface.VectorArray.full` methods
`[#612] <https://github.com/pymor/pymor/pull/612>`_. Vectors with random values can
be created using the :meth:`~pymor.vectorarrays.interface.VectorArray.random`
...
:meth:`~pymor.models.interface.Model.solve` method.
...
:class:`~pymor.models.interface.Model` `[#750] <https://github.com/pymor/pymor/pull/750>`_:
The output of a |Model| is defined to be a |VectorArray| of the model's
:attr:`!~pymor.models.interface.Model.output_space` |VectorSpace| and
can be computed using the new :meth:`~pymor.models.interface.Model.output` method.
Alternatively, :meth:`~pymor.models.interface.Model.solve` method can
...
:meth:`Operator <pymor.operators.interface.Operator.d_mu>` and
:meth:`ParameterFunctional <pymor.parameters.functionals.ParameterFunctional.d_mu>`
...
an implementation of :meth:`~pymor.operators.interface.Operator.restricted`
...
of :meth:`~pymor.vectorarrays.interface.VectorArray.amax` and
:meth:`~pymor.vectorarrays.interface.VectorArray.dofs` have been fixed
`[#616] <https://github.com/pymor/pymor/pull/616>`_ and
:attr:`~pymor.operators.interface.Operator.solver_options` are now correctly
handled in :meth:`!~pymor.operators.interface.Operator._assemble_lincomb`
...
implemented in the :mod:`~pymor.algorithms.rand_la` module
...
:meth:`~pymor.operators.interface.Operator.apply_inverse_adjoint` are implemented
...
`__repr__` implementation to `BasicObject` which recursively prints all class attributes
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
A new check in :class:`~pymor.core.base.ImmutableMeta` enforces all `__init__` arguments
of an |immutable| object to be available as object attributes, thus ensuring that
`~pymor.core.base.ImmutableObject.with_` works reliably with all |immutable| objects
...
method has been added to `BasicObject` `[#732] <https://github.com/pymor/pymor/pull/732>`_.
Finally, `~pymor.core.base.ImmutableObject.with_` now has a `new_type` parameter
...
code in :meth:`!~pymor.operators.interface.Operator._assemble_lincomb`
...
that when :meth:`~pymor.parameters.base.ParameterSpace.sample_randomly` is called
...
used by the newly introduced :meth:`~pymor.vectorarrays.interface.VectorArray.random`
method of the :class:`~pymor.vectorarrays.interface.VectorArray`
`[#620] <https://github.com/pymor/pymor/pull/620>`_.


Space id handling
~~~~~~~~~~~~~~~~~
The usage of |VectorSpace| :attr:`ids <pymor.vectorarrays.interface.VectorSpace.id>` in pyMOR
