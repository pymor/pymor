...
and :mod:`pymor.models.mpi` that allow an automatic
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The :meth:`~pymor.vectorarrays.interface.VectorArray.copy` method
...
- The :mod:`continuous Galerkin <pymor.discretizers.builtin.cg>` module has been extended to
  support Robin boundary conditions `[#110] <https://github.com/pymor/pymor/pull/110>`_.

- :class:`~pymor.analyticalproblems.functions.BitmapFunction` allows to use grayscale
...
- :class:`CacheRegions <pymor.core.cache.CacheRegion>` now have a
  ...
  `!~pymor.core.interface.BasicObject.uid` can be used instead.
  :attr:`pymor.core.cache.cache_regions` now by default contains `'memory'`,
...
- As an alternative to using the :meth:`@cached <pymor.core.cache.cached>`
  decorator, :meth:`~pymor.core.cache.CacheableObject.cached_method_call`
  can be used to cache the results of a function call. This is now used
  in :meth:`~pymor.models.interface.Model.solve`
...
^^^^^^^^^^^^^^^^^^^^^^^
- :meth:`~pymor.operators.interface.Operator.apply_inverse_adjoint` has been added to the |Operator| interface `[#133] <https://github.com/pymor/pymor/issues/133>`_.
...
- `from_file` factory method for |NumpyVectorArray| and |NumpyMatrixOperator| `[#118] <https://github.com/pymor/pymor/issues/118>`_.
    :meth:`NumpyVectorSpace.from_file <pymor.vectorarrays.numpy.NumpyVectorSpace.from_file>` and
...
- Default implementation of :meth:`~pymor.operators.interface.Operator.as_vector` for functionals `[#107] <https://github.com/pymor/pymor/issues/107>`_.
    :meth:`Operator.as_vector <pymor.operators.interface.Operator>` now
    contains a default implementation for functionals by calling
    :meth:`~pymor.operators.interface.Operator.apply_adjoint`.
...
- The `inner` method of the |VectorArray| interface has been split into :meth:`~pymor.vectorarrays.interface.VectorArray.inner` and :meth:`~pymor.vectorarrays.interface.VectorArray.pairwise_inner` `[#76] <https://github.com/pymor/pymor/issues/76>`_.
    The `pairwise` parameter of :meth:`~pymor.vectorarrays.interface.VectorArray.inner`
    has been removed, always assuming `pairwise == False`. The method
    :meth:`~pymor.vectorarrays.interface.VectorArray.pairwise_inner`
    corresponds to the `pairwise == True` case. Similarly the `pariwise` parameter
    of the :meth:`~pymor.operators.interface.Operator.apply2` method
    of the |Operator| interface has been removed and a
    :meth:`~pymor.operators.interface.Operator.pairwise_apply2` method
    has been added.
...
- Removal of the `options` parameter of :meth:`~pymor.operators.interface.Operator.apply_inverse` in favor of :attr:`~pymor.operators.interface.Operator.solver_options` attribute `[#122] <https://github.com/pymor/pymor/issues/122>`_.
    The `options` parameter of :meth:`Operator.apply_inverse <pymor.operators.interface.Operator.apply_inverse>`
    has been replaced by the :attr:`~pymor.operators.interface.Operator.solver_options`
    attribute. This attribute controls which fixed (linear) solver options are
    used when :meth:`~pymor.operators.interface.Operator.apply_inverse` is
...
- Caching of :meth:`~pymor.models.interface.Model.solve` is now disabled by default `[#178] <https://github.com/pymor/pymor/issues/178>`_.
    Caching of :meth:`~pymor.models.interface.Model.solve`
    must now be explicitly enabled by using
    :meth:`pymor.core.cache.CacheableObject.enable_caching`.
...
- `with_` has been moved from `BasicObject` to `ImmutableObject` `[#154] <https://github.com/pymor/pymor/issues/154>`_.

- `BasicObject.add_attributes` has been removed `[#158] <https://github.com/pymor/pymor/issues/158>`_.

- Auto-generated names no longer contain the :attr:`!~pymor.core.interface.BasicObject.uid` `[#198] <https://github.com/pymor/pymor/issues/198>`_.
    The auto-generated `~pymor.core.interface.BasicObject.name`
    of pyMOR objects no longer contains their
    `!~pymor.core.interface.BasicObject.uid`. Instead, the name
