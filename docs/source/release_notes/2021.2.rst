...
it is moved from :mod:`pymor.models.iosys` to
:mod:`pymor.models.transfer_function`.
Additionally, the `InputOutputModel` and `InputStateOutputModel` classes are
removed.
...
`transfer_function` should be used instead
(e.g., `m.bode_plot(...)` should be replaced with
