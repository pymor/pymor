...
`pymor.solvers.matrix.sylvester`. A generic low-rank ADI (Alternating Direction
Implicit) solver for Lyapunov equations is implemented in
:mod:`pymor.solvers.matrix.lradi`. Furthermore, bindings to low-rank and dense
...
:mod:`pymor.solvers.matrix.sylvester`.
